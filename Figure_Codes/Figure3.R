...
#                                             figure3. A                                   #
# ----------------------------------------------------------------------------------------- #
load('figure3_human.Rdata')
transcription_00_error <- gene_transcription_error[gene_transcription_error[,1]%in%geneid==F,]
translation_00_error <- gene_translation_error[gene_translation_error[,1]%in%cor_data[,1]==F,]
transcription_00_error <- transcription_00_error[,c(1,4)]
transcription_00_error[,3] <- transcription_00_error[,2]
transcription_00_error[,2] <- 0
colnames(transcription_00_error) <- colnames(cor_data)
translation_00_error <- translation_00_error[,c(1,4)]
translation_00_error[,3] <- 0
colnames(translation_00_error) <- colnames(cor_data)
cor_00_data <- rbind(cor_data,transcription_00_error,translation_00_error)
cor_00_data <- unique(cor_00_data)
...
for(i in seq(1,nrow(barcor_data_00)))
{
  ...
  {barcor_data_00[i,4] <- "1.8e-05"}
}
barcor_data_00$type <- factor(barcor_data_00$type, levels = c("1e-05","1.2e-05","1.4e-05","1.6e-05","1.8e-05"))
...
barcor_meandata_00$type <- factor(barcor_meandata_00$type, levels = c("1e-05","1.2e-05","1.4e-05","1.6e-05","1.8e-05"))
...
cor.test(barcor_meandata_00$translation_error,barcor_meandata_00$transcription_error,method = "pearson")
cor.test(barcor_meandata_00$translation_error,barcor_meandata_00$transcription_error,method = "spearman")
cor.test(cor_00_data$translation_error,cor_00_data$transcription_error,method = "pearson")
cor.test(cor_00_data$translation_error,cor_00_data$transcription_error,method = "spearman")
bar_100plot_00 <- ggplot(barcor_meandata_00, aes(x = as.numeric(type), y = translation_error)) +  
  geom_bar(stat = "identity", fill = "white", color = "grey35", width = 0.6, position = position_dodge(0.8)) +  # 设置空心柱子  
  ...
  geom_smooth(aes(x = as.numeric(type), y = translation_error), method = 'lm', se = FALSE, color = "black") +  # 将lm线改为黑色  
  scale_x_continuous(
    breaks = c(0.5, 1.5, 2.5, 3.5, 4.5,5.5),  # 刻度线放在两柱中间
    labels = c("[0.0]","[0.0,1.0]", "[1.0,1.2]", "[1.2,1.4]", "[1.4,1.6]", "[1.6,3.0]"),  # 设置标签
    limits = c(0.5, 5.7)
  ) +
  ...
  theme_classic() +  
  theme(axis.text.x = element_text(size = 23, angle = 45, vjust = 1, hjust = 1),  # 标签居中对齐  
...
for(i in seq(1,nrow(barcor_data_00)))
{
  ...
  {barcor_data_00[i,4] <- "1.4e-05"}
  if(barcor_data_00[i,3]>=1.4e-05 && barcor_data_00[i,3]<2e-05)
...
bar_100plot_00 <- ggplot(barcor_meandata_00, aes(x = as.numeric(type), y = translation_error)) +  
  ...
  scale_x_continuous(
    breaks = c(0.5, 1.5, 2.5, 3.5, 4.5,5.5),  # 刻度线放在两柱中间
    labels = c("[0.0]","[0.0,0.5]", "[0.5,0.8]", "[0.8,1.1]", "[1.1,1.4]","[1.4,1.7]"),  # 设置标签
    limits = c(0.5, 5.7)
  ) +
  scale_y_continuous(labels = c(expression(italic(0)),   
                                expression(2),   
...
for(i in seq(1,nrow(barcor_data_00)))
{
  ...
  if(barcor_data_00[i,3]>=2e-05 && barcor_data_00[i,3]<8e-05)
  {barcor_data_00[i,4] <- "2.5e-05"}
}
barcor_data_00$type <- factor(barcor_data_00$type, levels = c("5e-06","1e-05","1.5e-05","2e-05","2.5e-05"))
meannumber <- c("5e-06","1e-05","1.5e-05","2e-05","2.5e-05")
barcor_meandata_00 <- matrix(0,nrow = length(meannumber),ncol = 5)
colnames(barcor_meandata_00) <- c("gene","translation_error","transcription_error","type","se")
for(i in seq(1,length(meannumber)))
{
...
barcor_meandata_00 <- as.data.frame(barcor_meandata_00)
barcor_meandata_00$type <- factor(barcor_meandata_00$type, levels = c("5e-06","1e-05","1.5e-05","2e-05","2.5e-05"))
barcor_meandata_00[,2] <- as.numeric(barcor_meandata_00[,2])
barcor_meandata_00[,3] <- as.numeric(barcor_meandata_00[,3])
barcor_meandata_00[,5] <- as.numeric(barcor_meandata_00[,5])
cor.test(barcor_meandata_00$translation_error,barcor_meandata_00$transcription_error,method = "pearson")
cor.test(barcor_data_00$translation_error,barcor_data_00$transcription_error,method = "spearman")
cor.test(cor_00_data$translation_error,cor_00_data$transcription_error,method = "pearson")
cor.test(cor_00_data$translation_error,cor_00_data$transcription_error,method = "spearman")
bar_100plot_00 <- ggplot(barcor_meandata_00, aes(x = as.numeric(type), y = translation_error)) +  
  geom_bar(stat = "identity", fill = "white", color = "grey35", width = 0.6, position = position_dodge(0.8)) +  # 设置空心柱子  
  ...
  scale_x_continuous(
    breaks = c(0.5, 1.5, 2.5, 3.5, 4.5,5.5),  # 刻度线放在两柱中间
    labels = c("[0.0]","[0.0,0.5]", "[0.5,1]", "[1.0,1.5]", "[1.5,2.0]","[2.0,3.0]"),  # 设置标签
    limits = c(0.5, 5.7)
  ) +
  scale_y_continuous(labels = c(expression(italic(0)), 
                                expression(1), 
...
barcor_data_00$type <- factor(barcor_data_00$type, levels = c("8e-06","1.1e-05","1.4e-05","1.7e-05","2e-05"))
meannumber <- c("8e-06","1.1e-05","1.4e-05","1.7e-05","2e-05")
barcor_meandata_00 <- matrix(0,nrow = length(meannumber),ncol = 5)
colnames(barcor_meandata_00) <- c("gene","translation_error","transcription_error","type","se")
for(i in seq(1,length(meannumber)))
{
...
barcor_meandata_00 <- as.data.frame(barcor_meandata_00)
barcor_meandata_00$type <- factor(barcor_meandata_00$type, levels = c("8e-06","1.1e-05","1.4e-05","1.7e-05","2e-05"))
barcor_meandata_00[,2] <- as.numeric(barcor_meandata_00[,2])
barcor_meandata_00[,3] <- as.numeric(barcor_meandata_00[,3])
barcor_meandata_00[,5] <- as.numeric(barcor_meandata_00[,5])
...
bar_100plot_00 <- ggplot(barcor_meandata_00, aes(x = as.numeric(type), y = translation_error)) +  
  ...
  scale_x_continuous(
    breaks = c(0.5, 1.5, 2.5, 3.5, 4.5,5.5),  # 刻度线放在两柱中间
    labels = c("[0.0]","[0.0,0.8]", "[0.8,1.1]", "[1.1,1.4]", "[1.4,1.7]","[1.7,4.0]"),  # 设置标签
    limits = c(0.5, 5.7)
  ) +
  scale_y_continuous(labels = c(expression(italic(0)),   
                                expression(2),   
...
for(i in seq(1,nrow(barcor_data_00)))
{
  ...
  if(barcor_data_00[i,3]>=2.8e-05 && barcor_data_00[i,3]<3.5e-05)
...
bar_100plot_00 <- ggplot(barcor_meandata_00, aes(x = as.numeric(type), y = translation_error)) +  
  ...
  geom_smooth(aes(x = as.numeric(type), y = translation_error), method = 'lm', se = FALSE, color = "black") +  # 将lm线改为黑色  
  scale_x_continuous(
    breaks = c(0.5, 1.5, 2.5, 3.5, 4.5,5.5,6.5),  # 刻度线放在两柱中间
    labels = c("[0.0]","[0.0,2.0]", "[2.0,2.2]", "[2.2,2.4]", "[2.4,2.6]","[2.6,2.8]", "[2.8,3.0]"),  # 设置标签
    limits = c(0.5, 6.7)
  ) +
  scale_y_continuous(labels = c(expression(italic(0)),   
                                expression(2),   
...
OR_m_100_plot$Species <- factor(OR_m_100_plot$Species, 
                                levels = c("Human", "Mouse", "Drosophila", "C.elegans", "Yeast"),  # 按所需顺序排列
...
OR_100_plot <- ggplot(OR_m_100_plot, aes(x = logOR, y = Species)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "black",linewidth = 1) +  # 添加x轴虚线
