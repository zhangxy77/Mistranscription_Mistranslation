...
# 将数据从wide format转换为long format
species_mean_long <- melt(species_mean, id.vars = "type",
                          variable.name = "Species", value.name = "Value")
# 将标准差数据从wide format转换为long format
species_mean_sd_long <- melt(species_sd, id.vars = "type",
                             measure.vars = c("Human_sd", "Yeast_sd", "Mouse_sd", "Drosophila_sd", "Elegans_sd"),
                             variable.name = "Species", value.name = "SD")
...
ggplot(species_mean_long, aes(x = type, y = Value, fill = Species)) +  
  ...
  theme(  
    ...
    axis.title.x = element_text(size = 26, face = "bold"),  # 加粗 x 轴标题  
    axis.title.y = element_text(size = 26, face = "bold", margin = margin(l = 20, r = 20)),  # 加粗 y 轴标题  
