...
mclapply(
  ...
  function(thisParam){
    ...
    for(thisGen in c(1:num_generations)) {
      ...
      mut_effect1 <- log2(mut_effect1)
      mut_effect1 <- ifelse(mut_effect1 < 0,mut_effect1 * 0.5, mut_effect1);
      curPop$beta1 <- curPop$beta1 * ifelse(runif(nRow) < mut_rate, 2^mut_effect1,1);
      mut_effect2 <- rgamma(nRow, shape=shape_est_TL, scale=scale_est_TL);
      mut_effect2 <- log2(mut_effect2)
      mut_effect2 <- ifelse(mut_effect2 < 0,mut_effect1 * 0.5, mut_effect2);
      curPop$beta2 <- curPop$beta2 * ifelse(runif(nRow) < mut_rate, 2^mut_effect2,1);
      ...
      ind2fitness <- curPop %>%
        group_by(ind) %>%
        dplyr::summarise(
          totalErrCnt_single=sum(N * err1*(1-err2)+N*err2*(1-err1)),
          totalErrCnt_double=sum(N * err1*(1-err2)+N*e*err1*err2),
          totalErrCnt = sum(N * err1*(1-err2)+N*err2*(1-err1)+N*e*err1*err2),  ## toxicity from error molecules
        ) %>%
        mutate(fitness = exp(-param_c * totalErrCnt));
      ...
      trackFitness_data[[thisGen]] <- ind2fitness$fitness
      if(thisGen==1)
      {relativeFitness[[thisGen]] <- trackFitness_data[[thisGen]]/mean(trackFitness_data[[thisGen]]) }
      if(thisGen>1)
      {relativeFitness[[thisGen]] <- trackFitness_data[[thisGen]]/mean(trackFitness_data[[thisGen-1]]) }
      ...
      if(thisGen %% 200 == 0) {
        save(file=filename,list=c("filename","pop","trackFitness","trackCor","mut_rate","param_c","thisGen","trackFitness_data","relativeFitness"));
