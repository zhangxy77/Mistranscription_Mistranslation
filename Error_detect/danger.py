    ...
    for spec in mod.findall('specificity'):
        ...
        if spec.find('NeutralLoss') is not None:
            for neutral_loss in spec.findall('NeutralLoss'):
                ...
                lines.append({'title': '%s - %s' %(title,comp),
...
mods.reset_index(drop=True).to_pickle(r'danger_mods')

mods.reset_index(drop=True).to_csv(r'danger_mods.csv')
